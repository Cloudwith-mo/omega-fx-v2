...
def main():
    ...
    kd_risks = [0.1, 0.2, 0.3]
    vv_risks = [0.2, 0.3, 0.4]
    big_risks = [0.2, 0.3, 0.4]
    dd_thresholds = [0.04, 0.05, 0.07]
    reduction_factors = [0.3, 0.5, 0.7]

    candidates = list(itertools.product(london_risks, momentum_risks, kd_risks, vv_risks, big_risks, dd_thresholds, reduction_factors))
    random.shuffle(candidates)
    total = len(candidates)
    start_time = time.time()

    scored = []
    for idx, (l_risk, m_risk, kd_risk, vv_risk, big_risk, dd_th, red_fac) in enumerate(candidates, 1):
        ...
        risk_scales = {
            ...
            EXPERIMENTAL_PROFILE_USDJPY_VANVLEET_M15_V1.name: vv_risk,
            EXPERIMENTAL_PROFILE_USDJPY_BIGMAN_H1_V1.name: big_risk,
        }
        ...
        scored.append(
            {
                ...
                "kd_risk": kd_risk,
                "vanvleet_risk": vv_risk,
                "bigman_risk": big_risk,
    ...
    out_path = ROOT / "reports" / "optimize_usdjpy_exp_v3_risk.json"
