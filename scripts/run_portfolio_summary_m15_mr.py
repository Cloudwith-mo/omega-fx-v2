from omegafx_v2.config import DEFAULT_PORTFOLIO_M15_MR_MULTI
from omegafx_v2.portfolio_summary import run_portfolio_summary


def main() -> None:
    portfolio = DEFAULT_PORTFOLIO_M15_MR_MULTI
    ...
    print("\nStrategy                        Symbol  TF    PassRate   AvgRet   MaxDD   AvgTrades  Signals")
    print("-------------------------------------------------------------------------------------------")
    for s in metrics["strategies"]:
        print(
            f"{s['name'][:30]:30s}  {s['symbol']:6s}  {s['timeframe']:4s}  "
