...
def prepare_events(profiles, ohlc_map):
    ...
    for idx, profile in enumerate(profiles):
        ...
        if USE_REGIME_MASKS and profile.edge_regime_config:
            regime_series = ohlc_map[profile.symbol_key].get("regime")
            if regime_series is not None:
                ...
                regime_mask = regime_series.map(lambda r: allowed_map.get(r, False)).reindex(signals.index, method="ffill").fillna(False)
...
def main() -> None:
    ...
    if USE_REGIME_MASKS:
        ohlc_map["USDJPY"]["regime"] = tag_regimes(
            ohlc_m15_usd, compute_atr(ohlc_m15_usd, period=14), ohlc_m15_usd["close"].ewm(span=50, adjust=False).mean().diff()
        )
        ohlc_map["GBPJPY"]["regime"] = tag_regimes(
            ohlc_m15_gbp, compute_atr(ohlc_m15_gbp, period=14), ohlc_m15_gbp["close"].ewm(span=50, adjust=False).mean().diff()
        )
    ...
    for i in range(num_runs):
        # Emit structured progress for dashboard consumption
    ...
    print("=== FTMO Multi (USDJPY+GBPJPY) – Full Pipeline ===")
    ...
    print(f"Pipeline success rate (Phase1 & Phase2 pass): {len(pipeline_success)/len(results) if results else 0.0:.2%}")
    ...
    for r in live_ok:
        ...
        max_dds.append(
            max(
                r["phase1"]["max_drawdown_pct"],
                r["phase2"]["max_drawdown_pct"] if r.get("phase2") else 0,
                r["live"]["max_drawdown_pct"] if r.get("live") else 0,
            )
        )
    ...
    if live_ok:
        ...
        if days_stats:
            summary["total_days_to_payout_avg"] = days_stats.get("avg")
            summary["total_days_to_payout_median"] = days_stats.get("median")
            summary["total_days_to_payout_p90"] = days_stats.get("p90")
        if trades_stats:
            summary["trades_to_payout_avg"] = trades_stats.get("avg")
            summary["trades_to_payout_median"] = trades_stats.get("median")
            summary["trades_to_payout_p90"] = trades_stats.get("p90")
        if max_dds:
            summary["avg_max_dd"] = float(np.mean(max_dds))
            summary["worst_max_dd"] = float(np.max(max_dds))

    out_path = reports_dir / "ftmo_multi_usdjpy_gbpjpy_full_pipeline.json"
